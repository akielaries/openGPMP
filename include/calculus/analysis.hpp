...
class NumericalAnalysis {
  public:
    ...
    */
    ...
    double linearApproximation(double (*function)(double), double c,
                               double x);

    double quadraticApproximation(double (*function)(double), double c,
                                  double x);

    double cubicApproximation(double (*function)(double), double c,
                              double x);
    ...
    double growthMethod(
        double C, double k,
        double t); // General growth-based diffrential equations can
                   // be solved by seperation of variables.
    ...
    std::vector<std::vector<double>>
        hessian(double (*function)(std::vector<double>),
                std::vector<double> x);
    ...
    std::string secondPartialDerivativeTest(
        double (*function)(std::vector<double>), std::vector<double> x);
};
} // namespace mtpk
