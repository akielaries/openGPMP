...
void main() {
    ...
    // So we want to set bit 4 of this register to 1 to enable port C, but where
    // is this register in memory? We're only being told that it is at address
    // 0x18 within the memory for RCC configuration. So let's go back to
    // section 3.3 to consult the memory map. There, "Reset and clock control
    // RCC" is listed at 0x4002 1000. Adding 0x18 we get 0x4002 1018.
    *((volatile unsigned int *)0x40021018) |= (1 << 4);
    ...
    // config of any other pins. The section specifies an address offset of
    // 0x04, but what is the base for this? Our trusty memory map in section 3.3
