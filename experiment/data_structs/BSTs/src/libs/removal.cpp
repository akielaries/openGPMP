...
/*
Name: removeLeafNode
Process: removes a leaf node from the tree, frees the removed leaf node from memory
Function input/parameters: wkgPtr (current node in tree),
                           searchRank (int, rank of the node that should be removed)
Function output/parameters: none
...
PasswordType *remove_leaf_node(PasswordType *wkgPtr, int searchRank) {
    ...
    // otherwise
        ...
        // get inorder next
        PasswordType *tempPtr = find_smallest_leaf_node(currentNode->rightChildPtr);
