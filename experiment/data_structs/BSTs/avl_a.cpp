...
PasswordType *removeItemHelper(PasswordType *wkgPtr, PasswordType *toRemovePtr);
PasswordType *search(PasswordType *wkgPtr, PasswordType *toRemovePtr);
PasswordType *removeFromMax(PasswordType *parentPtr, PasswordType *childPtr);
    ...
    if(size >= 1 && size <= MAX_NODES)
    {
        ...
        if(rootPtr != NULL)
        {
           ...
           // remove some nodes
            ...
            printf(
            "\nHeight difference of root node subtrees after removals: %d\n",
                                                                    heightDiff);
...
int getBalanceFactor(PasswordType *localPtr)
{
    ...
    return findSubTreeHeight(localPtr->leftChildPtr) - findSubTreeHeight(localPtr->rightChildPtr);
}
...
PasswordType *rebalanceTree(PasswordType *localPtr)
{
    ...
    if (currentNodeHeightDifference > 1) 
    {
        ...
        leftChildHeightDifference = getBalanceFactor(localPtr->leftChildPtr);
    ...
    else if (currentNodeHeightDifference < -1)
    {
        ...
        if (getBalanceFactor(localPtr->rightChildPtr->rightChildPtr) <= 0)
        {
            //  rotate to rebalance
...
/*
Name: removeFromMax
Process: recursively traverses for max node. When found, unlinks node from tree and returns it.
Function input/parameters: pointer to parent and child nodes 
Function output/parameters: none
...
/*
Name: removeItem
Process: searches for item to be removed, if item is present then helper function is called
to remove it. Otherwise, a message is displayed to the console indicating the item
is not present.
Function input/parameters: rootPtr of tree, pointer to data that should be removed
Function output/parameters: none
...
Process: recursively searches for item, removes node,
         ...
         rebalances the tree and returns updated link to parent (at each recursive level)
Function input/parameters: rootPtr of tree, pointer to data that should be removed
Function output/parameters: none
Function output/returned: link to recursive parent
Device input/file: none
Device output/monitor: none
Dependencies: rebalanceTree, removeItemHelper, removeFromMax, setNodeDataFromNode, free
*/
PasswordType *removeItemHelper(PasswordType *wkgPtr, PasswordType *toRemovePtr)
{
    // initialize variables
    ...
    if (wkgPtr != NULL)
    {
        // check if the the data to be removed is less than the current node data
        if(toRemovePtr->rank < wkgPtr->rank)
        {
            // recurse to the left
           wkgPtr->leftChildPtr = removeItemHelper(wkgPtr->leftChildPtr,
                                                                  toRemovePtr);
        }
        // otherwise, check to see if removal data is larger than the data at the current node
        else if(toRemovePtr->rank > wkgPtr->rank)
        {
            // recurse to the right
            wkgPtr->rightChildPtr = removeItemHelper(wkgPtr->rightChildPtr,
                                                                   toRemovePtr);
        }
        ...
        else
        {
            ...
            else
            {
                ...
                if (wkgPtr->leftChildPtr->rightChildPtr != NULL)
                {
                    ...
                    temp = removeFromMax(wkgPtr,wkgPtr->leftChildPtr->rightChildPtr);
                    // copy data from removeFromMax into current node
                       //function: setNodeDataFromData
                    // setNodeDataFromNode(wkgPtr, wkgPtr->leftChildPtr->rightChildPtr);
                    setNodeDataFromNode(wkgPtr, temp);
                    // free the node that was unlinked (tempPtr)
                    free(temp);
                }
                // otherwise, current node's left child does not have a right child
                else
                {
...
/*
Name: search
Process: searches for data, if data is present function returns a pointer to node,
 if node is not found, function returns NULL.
Function input/parameters: rootPtr of tree, pointer to data that should be removed
Function output/parameters: none
Function output/returned: pointer to node if found, NULL if node is not found.
Device input/file: none
Device output/monitor: none
...
PasswordType *search(PasswordType *wkgPtr, PasswordType *toRemovePtr)
{
    ...
    if (wkgPtr != NULL)
    {
        // check to see if the data at toRemovePtr is less than the rank at wkgPtr
        if (toRemovePtr->rank < wkgPtr->rank)
        {
...
PasswordType *insert(PasswordType *wkgPtr, PasswordType *inNodePtr)
{
    ...
    if(wkgPtr != NULL)
    {
       ...
       printf("\n    Height difference: %d\n", heightDifference);
        ...
        if(heightDifference > 1 && inNodePtr->rank < wkgPtr->leftChildPtr->rank)
        {
            printf("\n    left left case\n");
        ...
        else if(heightDifference < -1 && inNodePtr->rank > wkgPtr->rightChildPtr->rank)
        {
          printf("\n    right right case\n");
      ...
      else if(heightDifference > 1 && inNodePtr->rank > wkgPtr->leftChildPtr->rank)
      {
          printf("\n    left right case\n");
          printf("\n      --rotating left on node with rank %d\n",
