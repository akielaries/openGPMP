...
void *key_to_hash(const char *key) {
    ...
    // CODE HERE (loop)
    for (int current_char = 0; current_char < strlen(key);
         current_char++) {
...
const char *hash_table_set(hash_table *table, const char *key,
    ...
    // CODE HERE
    return hash_table_set_entry(table->entries, table->capacity, key,
                                value, &table->length);
}
...
static bool hash_table_expand(hash_table *table) {
    ...
    hash_table_entry *new_entries =
        calloc(new_capacity, sizeof(hash_table_entry));
    ...
    // Iterate all old entries,and move all non-empty ones to new table's
    // entries. CODE HERE (for)
    for (int cutrrent_entry = 0; cutrrent_entry < table->capacity;
         cutrrent_entry++) {
