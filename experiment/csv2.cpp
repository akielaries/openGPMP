...
void csv_read(const char *fname,
...
int main() {
    ...
    std::cout << "Time to read and store CSV data: " << elapsed_time.count()
              << " ms\n";
...
void csv_read(const char *fname,
    ...
    // Count the number of lines in the CSV file and calculate the number of
    // columns
    ...
    for (auto currentPos = csvData; currentPos && currentPos != endOfFile;
         ++currentPos) {
        if (*currentPos == '\n') {
    ...
    for (; currentPos && currentPos != endOfFile; ++currentPos) {
        if (*currentPos == ',' || *currentPos == '\n') {
            ...
            for (; columnStart != currentPos; ++columnStart) {
                if (*columnStart == ',') {
                    std::string value(columnStart + 1,
                                      currentPos - columnStart - 1);

                    try {
                        // Try converting to integer
                        int64_t int_value = std::stoll(value);
                        columns[columnIndex++].emplace_back(
