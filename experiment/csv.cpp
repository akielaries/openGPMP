...
public:
    MyDataFrame(const std::string& filename) {
        ...
        while (getline(file, line)) {
            ...
            while (getline(rowStream, value, ',')) {
                if (columnIndex >= headerCols.size()) {
                    // Handle the case where the number of columns in the row exceeds the header
                    break;
                }
        ...
        std::cout << "Time taken for CSV reading: " << duration.count() << " ms" << std::endl;

        // Log column types
        std::cout << "Column Types:" << std::endl;
        std::cout << "intColumns: " << intColumns.size() << " elements" << std::endl;
        std::cout << "doubleColumns: " << doubleColumns.size() << " elements" << std::endl;
        std::cout << "stringColumns: " << stringColumns.size() << " elements" << std::endl;
    ...
    void printFirst5Rows() {
        size_t rowCount = std::min(std::min(std::min(intColumns.size(), doubleColumns.size()), stringColumns.size()), size_t(5));
